...
class SoftmaxLoss(Module):
    def forward(self, logits: Tensor, y: Tensor):
        ...
        z_y_sum = (
            logits * init.one_hot(logits.shape[1], y, logits.device, logits.dtype)).sum()
...
class BatchNorm1d(Module):
    def __init__(self, dim, eps=1e-5, momentum=0.1, device=None, dtype="float32"):
        ...
        self.weight = Parameter(
            init.ones(self.dim, device=device, dtype=dtype, requires_grad=True))
        self.bias = Parameter(init.zeros(
            self.dim, device=device, dtype=dtype, requires_grad=True))
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.training:
            ...
            x_std = ((var + self.eps) ** 0.5).reshape((1,
                                                       x.shape[1])).broadcast_to(x.shape)
...
class LayerNorm1d(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x_std = ((x_minus_mean ** 2).sum(axes=(1, )
                                         ).reshape((batch_size, 1)) / feature_size + self.eps) ** 0.5
        # NOTE need manual broadcast_to!!!
        normed = x_minus_mean / \
            x_std.reshape((x.shape[0], 1)).broadcast_to(x.shape)
    ...
    def forward(self, X, h=None):
        ...
        if self.bias:
            out += self.bias_ih.reshape((1, self.hidden_size)
                                        ).broadcast_to((bs, self.hidden_size))
        if h is not None:
            out += h @ self.W_hh
        if self.bias:
...
class RNN(Module):
    def __init__(self, input_size, hidden_size, num_layers=1, bias=True, nonlinearity='tanh', device=None, dtype="float32"):
        ...
        for i in range(num_layers - 1):
            rnn_cells.append(RNNCell(hidden_size, hidden_size,
...
class Embedding(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        one_hot_vectors = self.one_hot = init.one_hot(
