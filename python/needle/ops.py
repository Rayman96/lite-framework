...
class ReLU(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ...
        mask = Tensor(a > 0, device=out_grad.device,
                      dtype=out_grad.dtype, requires_grad=False)
...
class LogSumExp(TensorOp):
    ...
    def compute(self, Z):
        ...
        ret = array_api.log(array_api.exp(
            Z - maxz.broadcast_to(Z.shape)).sum(axis=self.axes, keepdims=True)) + maxz
        if self.axes is None:
        ...
        if self.axes is not None:
            out_shape = [size for i, size in enumerate(
                Z.shape) if i not in axes]
        else:
            out_shape = [1]
...
class Split(TensorTupleOp):
    ...
    def compute(self, A):
        ...
        slices = [slice(0, A.shape[i], 1) if i !=
                  self.axis else 0 for i in range(len(A.shape))]
        tensors = []
        new_shape = tuple([A.shape[s]
                          for s in range(len(A.shape)) if s != self.axis])
